...
class Vss {
    ...
    fatal(msg: string) {
        this.bridge().fatal("vss: " + msg);
    }

    getScriptsFolder = () => this.bridge().getScriptsFolder();
    initScripts = (folder: string) => this.bridge().initScripts(folder);
    sendEvent = (code: actEventCodes, data?: number) => this.bridge().sendEvent(code, data);
    isKeyPressed = (scanCode: number) => this.bridge().isKeyPressed(scanCode);
    isFileExists = (file: string) => this.bridge().isFileExists(file);
    getRgbaData = (
        frame: Uint8Array,
        frameWidth: number,
        startX: number,
        ...
        height: number,
        rgbaData: Uint8Array,
    ) => this.bridge().getRgbaData(frame, frameWidth, startX, startY, width, height, rgbaData);
    toBase64 = (data: Uint8Array) => this.bridge().toBase64(data);
    getShopItem = () => this.bridge().getShopItem();
    renderLine = (line: number) => this.bridge().renderLine(line);
    getLineT = (line: number) => this.bridge().getLineT(line);
    ...
    private bridge() {
        return global.bridge as VssNative;
    }
}
