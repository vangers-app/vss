...
{
	return {rv_model_create(_context, name, model)};
}
...
{
	return {rv_model_instance_create(_context, model_handle.handle, color_id)};
}
...
void RustVisualBackend::model_instance_set_transform(ModelInstanceHandle model_instance_handle, const Transform& transform)
{
	rv_transform v_transform {
		.position = rv_vector3 {
			.x = transform.position.x,
