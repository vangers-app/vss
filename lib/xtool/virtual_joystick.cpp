...
private:
	...
	constexpr static std::array<Sprite, 9> sprites{
...
struct DirectionPad final : public BaseWidget {
	...
	void process(const TouchEvent &event, KeyMap &keyMap) override {
		...
		const auto threshold = frame.w / 4;
		const auto x = event.x - frame.x;
		const SDL_EventType eventType = event.type == SDL_FINGERUP ? SDL_KEYUP : SDL_KEYDOWN;
		if (x > (frame.w / 2 + threshold)) {
			keyMap[SDL_SCANCODE_RIGHT] = eventType;
		} else if (x < (frame.w / 2 - threshold)) {
			keyMap[SDL_SCANCODE_LEFT] = eventType;
		} else {
			keyMap[SDL_SCANCODE_LEFT] = SDL_KEYUP;
			keyMap[SDL_SCANCODE_RIGHT] = SDL_KEYUP;
		}

		const auto y = event.y - frame.y;
		if (y > (frame.h / 2 + threshold)) {
			keyMap[SDL_SCANCODE_DOWN] = eventType;
		} else if (y < (frame.h / 2 - threshold)) {
			keyMap[SDL_SCANCODE_UP] = eventType;
		} else {
			keyMap[SDL_SCANCODE_UP] = SDL_KEYUP;
			keyMap[SDL_SCANCODE_DOWN] = SDL_KEYUP;
		}
...
class Screen final {
public:
	Screen(SDL_Renderer *renderer, int screenWidth, int screenHeight)
	: m_renderer(renderer), m_screenWidth(screenWidth), m_screenHeight(screenHeight), m_atlas(renderer) {
	...
	void draw() {
		...
		if (!m_hasLayout) {
			m_root->adjustSize();
			m_root->layout(SDL_Rect{ 0, 0, m_screenWidth, m_screenHeight });
			m_hasLayout = true;
		}
...
struct VirtualJoystick::Context final {
	Context(SDL_Renderer *renderer, int screenWidth, int screenHeight)
	: screen(renderer, screenWidth, screenHeight) {}
...
void VirtualJoystick::setup(SDL_Renderer *renderer, int screenWidth, int screenHeight) {
	m_context = std::make_unique<Context>(renderer, screenWidth, screenHeight);
}
